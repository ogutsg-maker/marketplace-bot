...
Client discovery -> selection -> negotiation -> test payment -> booking ->
partner data -> partner cabinet/check-in/finance.

The module deliberately uses the existing PostgreSQL platform tables and the
existing booking schema. Idram is TEST mode only until a real provider is
connected.
"""
...
def _uid(request):
    ...
    try:
        data = validate_telegram_webapp_init_data(raw, request.app.get("stage3_bot_token") or os.getenv("BOT_TOKEN", ""))
        return int(data["id"])
...
def _commission(service):
    ...
    if isinstance(data, str):
        try:
            ...
            data = {}
    try:
        rate = float(data.get("commission_percent", 10))
    except Exception:
        rate = 10.0
...
async def client_search(request):
    ...
    if city:
        params.append(city)
        where.append("EXISTS (SELECT 1 FROM partner_locations pl WHERE pl.partner_id=p.id AND LOWER(COALESCE(pl.city,''))=LOWER(%s))")
    if category_id:
        params.append(category_id)
        where.append("s.category_id=%s")
    sql = f"""
        SELECT s.id service_id,s.partner_id,s.category_id,s.name service_name,s.description,
               s.price,s.currency,s.duration_minutes,s.data_json,
               p.business_name,p.business_description,p.contact_share_policy,
               c.name_am category_name_am,c.name_ru category_name_ru,
               COALESCE((SELECT pl.city FROM partner_locations pl WHERE pl.partner_id=p.id ORDER BY pl.id LIMIT 1),'') city,
               COALESCE((SELECT pl.marz FROM partner_locations pl WHERE pl.partner_id=p.id ORDER BY pl.id LIMIT 1),'') marz
        FROM services s
        ...
        GROUP BY s.id,p.id,c.id,p.business_name,p.business_description,p.contact_share_policy
...
async def create_request(request):
    ...
    item = _exec("""
        INSERT INTO service_requests(client_id,status,language,city,summary,preferences_json)
        VALUES(%s,'searching',%s,%s,%s,%s::jsonb) RETURNING *
    """, (uid, lang, city, summary, _json(profile)), True)
...
async def select_candidate(request):
    ...
    item = _one("""
        SELECT sr.id request_id,sr.status,s.id service_id,s.partner_id,s.name service_name,s.price,
               s.currency,s.duration_minutes,p.business_name,p.contact_share_policy
    ...
    """, (service_id, rid, uid))
    if not item:
        return web.json_response({"ok": False, "error": "candidate_not_found"}, status=404)
    _exec("""INSERT INTO request_candidates(request_id,partner_id,service_id,rank_score,status)
              VALUES(%s,%s,%s,100,'selected') ON CONFLICT(request_id,partner_id,service_id)
              DO UPDATE SET status='selected'""", (rid,item["partner_id"],service_id))
    negotiation = _exec("""
        INSERT INTO negotiations(request_id,client_id,partner_id,state_json)
        VALUES(%s,%s,%s,%s::jsonb)
        RETURNING *
    """, (rid,uid,item["partner_id"],_json({"service_id": service_id, "service_name": item["service_name"], "price": float(item["price"] or 0), "currency": item["currency"]})), True)
    _exec("UPDATE service_requests SET status='negotiating',updated_at=NOW() WHERE id=%s", (rid,))
    _exec("INSERT INTO negotiation_messages(negotiation_id,sender_role,sender_id,message,data_json) VALUES(%s,'ai',NULL,%s,%s::jsonb)", (negotiation["id"], "Ընտրված ծառայությունն է՝ %s։ Գինը՝ %s %s։ Կարող եք գրել ձեր ցանկությունները կամ համաձայնել։" % (item["service_name"], item["price"], item["currency"]), _json({"type":"selection"})))
...
async def negotiation_get(request):
    ...
    n = _one("SELECT * FROM negotiations WHERE id=%s AND client_id=%s", (nid,uid))
    if not n:
        return web.json_response({"ok":False,"error":"negotiation_not_found"},status=404)
    messages = _rows("SELECT id,sender_role,sender_id,message,data_json,created_at FROM negotiation_messages WHERE negotiation_id=%s ORDER BY id", (nid,))
    return web.json_response({"ok":True,"negotiation":n,"messages":messages})


async def negotiation_client_message(request):
    uid = _uid(request)
    nid = int(request.match_info["negotiation_id"])
    data = await request.json(); text = str(data.get("message") or "").strip()
    ...
    low=text.lower()
    if any(x in low for x in ("соглас", "беру", "да", "согласен", "համաձայն", "այո", "agree")):
        _exec("UPDATE negotiations SET status='agreed',updated_at=NOW() WHERE id=%s",(nid,))
        _exec("UPDATE service_requests SET status='confirmed',updated_at=NOW() WHERE id=%s",(n["request_id"],))
        reply="Պայմանները համաձայնեցված են։ Հաջորդ քայլը՝ Test Idram վճարումը և ամրագրումը։"
    else:
        reply="Ձեր հարցումը փոխանցված է գործընկերոջը։ Գործընկերը կարող է պատասխանել իր կաբինետից։"
    _exec("INSERT INTO negotiation_messages(negotiation_id,sender_role,sender_id,message) VALUES(%s,'ai',NULL,%s)",(nid,reply))
    return await negotiation_get(request)


async def partner_negotiations(request):
    uid = _uid(request)
    partner = _one("SELECT * FROM partners WHERE user_id=%s",(uid,))
    if not partner: return web.json_response({"ok":False,"error":"partner_not_found"},status=404)
    items=_rows("""
      SELECT n.*,sr.summary,sr.city,s.name service_name,p.business_name
      ...
      WHERE n.partner_id=%s AND n.status='active' ORDER BY n.updated_at DESC
    """,(partner["id"],))
    for x in items:
        x.pop("state_json",None)
    return web.json_response({"ok":True,"items":items})


async def partner_negotiation_messages(request):
    uid=_uid(request); nid=int(request.match_info["negotiation_id"])
    n=_one("SELECT n.* FROM negotiations n JOIN partners p ON p.id=n.partner_id WHERE n.id=%s AND p.user_id=%s",(nid,uid))
...
async def partner_agree(request):
    ...
    _exec("UPDATE negotiations SET status='agreed',updated_at=NOW() WHERE id=%s",(nid,))
    _exec("UPDATE service_requests SET status='confirmed',updated_at=NOW() WHERE id=%s",(n["request_id"],))
    _exec("INSERT INTO negotiation_messages(negotiation_id,sender_role,sender_id,message) VALUES(%s,'partner',%s,%s)",(nid,uid,"Համաձայն եմ։ Կարող ենք ամրագրել։"))
    return web.json_response({"ok":True,"status":"agreed"})


async def test_payment(request):
    uid=_uid(request); nid=int(request.match_info["negotiation_id"]); data=await request.json()
    ...
    state=n.get("state_json") or {}; service_id=int(state.get("service_id") or 0)
    ...
    price=float(data.get("amount") or service.get("price") or 0)
    rate=_commission(service); commission=round(price*rate/100,2); partner_amount=round(price-commission,2)
    booking=_exec("""
      INSERT INTO bookings(request_id,negotiation_id,client_id,partner_id,service_id,status,service_name,agreed_price,currency,commission_amount,partner_amount,data_json)
      VALUES(%s,%s,%s,%s,%s,'paid',%s,%s,%s,%s,%s,%s::jsonb) RETURNING *
    """,(n["request_id"],nid,uid,n["partner_id"],service_id,service["name"],price,service["currency"],commission,partner_amount,_json({"payment_mode":"TEST_IDRAM","test_transaction":"TEST-IDRAM-"+secrets.token_hex(6),"service_price":price,"commission_percent":rate})),True)
    payment=_exec("""
      INSERT INTO payments(booking_id,client_id,partner_id,payment_type,status,amount,currency,provider,provider_payment_id,data_json)
      VALUES(%s,%s,%s,'commission','paid',%s,%s,'IDRAM_TEST',%s,%s::jsonb) RETURNING *
    """,(booking["id"],uid,n["partner_id"],commission,service["currency"],booking["data_json"].get("test_transaction"),_json({"test":True})),True)
    ...
    if partner.get("contact_sharing_enabled"):
        contact={"phone":profile.get("phone"),"website":profile.get("website"),"telegram":profile.get("telegram")}
