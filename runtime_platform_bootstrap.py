...
async def _admin_document_proxy(request: web.Request):
    ...
    pid = int(request.match_info["id"]); doc_id = int(request.match_info["doc_id"])
    ...
    if row.get("file_data") is not None:
        return web.Response(body=bytes(row["file_data"]), content_type=mime, headers={"Content-Disposition": f'inline; filename="{filename}"'})
    ...
    try:
        ...
        return web.Response(body=data, content_type=mime, headers={"Content-Disposition": f'inline; filename="{filename}"'})
...
async def _legacy_document_open(request: web.Request):
    ...
    if not row:
        return web.json_response({"ok": False, "error": "document_not_found"}, status=404)
    return web.json_response({"ok": True, "admin_id": admin_id, "url": _document_download_url(pid, doc_id), "source": "proxy"})


def _bootstrap(app: web.Application) -> None:
    main = importlib.import_module("__main__")
    db = getattr(main, "db", None); ai = getattr(main, "ai", None); bot = getattr(main, "bot", None)
